...
import AIChatbot from "../../Chat/AIChatbot"; // Ensure this import is correct
...
const MentorDashboard = () => {
    ...
    useEffect(() => {
        ...
        // Cleanup LocalStorage Chat state on mount
    ...
    useEffect(() => {
        const playNotifySound = () => new Audio(NotifySound).play();

        const handleMsgRec = (data) => {
            const chatId = data.data.chat._id.toString();
            const isChatOpen = JSON.parse(localStorage.getItem("chatRoute"));
            const selectedChat = localStorage.getItem("selectedChat");

            const currentChats = JSON.parse(localStorage.getItem("chats") || "[]");
            if (!currentChats.some(c => c._id === chatId)) {
                dispatch(addSingleChat(data.data.chat));
            }

            if (selectedChat === chatId) {
                if (JSON.parse(localStorage.getItem("visible")) || !isChatOpen) {
                    dispatch(addNotification(chatId));
                    dispatch(reorderChats(chatId));
                    playNotifySound();
                    if (!isChatOpen) setNewMsgNotify(true);
                }
        ...
        if (role !== Roles.ADMIN) socket.on("message received", handleMsgRec);
        return () => socket.off("message received", handleMsgRec);
        ...
        <authContext.Provider value={{ uid, role }}>
            <div className="h-screen flex bg-gray-50 overflow-hidden">
                <nav className="w-3/20 h-screen bg-white flex flex-col z-10">
                    ...
                    {role === Roles.ADMIN && (
                        <>
                            <button id="manageGroups" onClick={handleRouteChange} className="flex items-center justify-between bg-blue-600 hover:bg-blue-800 text-white m-4 p-3 rounded-md">
                                Manage groups <Plus alt={true} myStyle="h-6 w-6 text-white" />
                            </button>
                            <button id="createUsers" onClick={handleRouteChange} className="flex items-center justify-between bg-blue-600 hover:bg-blue-800 text-white m-4 p-3 rounded-md">
                                Create Users <Plus alt={true} myStyle="h-6 w-6 text-white" />
                            </button>
                        </>
                    )}
                    ...
                    <button onClick={handleLogout} className="flex items-center text-left hover:bg-red-200 text-gray-800 mt-auto m-4 p-3 rounded-md bg-red-100 transition-all">
                        <LogoutIcon myStyle="h-5 w-5 mr-3 text-red-600" alt={true} /> Logout
                    </button>
                </nav>
                ...
                <div className="w-17/20 h-screen">
                    <Header 
                        role={role} 
                        data={role === Roles.ADMIN ? adminData : profileData} 
                        onNotiClick={() => setShowNotificationDropDown(!showNotificationDropDown)}
                        showNoti={showNotificationDropDown}
                    />
    ...
    <div className="relative w-full h-1/10 bg-white shadow-md flex items-center justify-end px-8">
        <div className="flex items-center gap-x-6">
            ...
            <div className="flex items-center gap-x-3">
                <img 
                    src={data?.avatar?.url || `https://api.dicebear.com/9.x/personas/svg`} 
                    alt="avatar" 
...
const NotificationElements = ({ showOverlay, overlayRef, showNotificationModal, notificationModalRef, modalContent, setShowNotificationModal, setShowOverlay, showNotificationDropDown, notificationDropDownRef, setModalContent }) => (
    <>
        ...
        <CSSTransition nodeRef={notificationModalRef} in={showNotificationModal} timeout={300} classNames="modal" unmountOnExit>
            <NotificationModal nodeRef={notificationModalRef} setShowNotificationModal={setShowNotificationModal} setShowOverlay={setShowOverlay} notification={modalContent} />
        </CSSTransition>
        <CSSTransition nodeRef={notificationDropDownRef} in={showNotificationDropDown} timeout={300} classNames="modal" unmountOnExit>
            <Notification nodeRef={notificationDropDownRef} setShowNotificationModal={setShowNotificationModal} setShowOverlay={setShowOverlay} setModalContent={setModalContent} />
        </CSSTransition>
    </>
);
